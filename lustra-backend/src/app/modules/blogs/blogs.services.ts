...
import { TQueryObj, TUpdateCourse } from './blogs.interface';

// eslint-disable-next-line no-unused-vars, @typescript-eslint/no-unused-vars
const getAllBlogService = async (query: TQueryObj) => {
  const result = await Blog.find({}).populate('author').limit(50);
  return result;
};
const getSingleBlogService = async (id: string) => {
  ...
  const comments = await Comment.find({ blogId: id }).populate('commentedBy');
